...
use App\Http\Controllers\Agent\SetupPinController;
...
Route::prefix("agent")->name("agent.")->group(function(){
    ...
    //Send Money
    Route::middleware(['module:agent-transfer-money','agent.pin.setup.guard'])->group(function(){
    ...
    //Money In
    Route::middleware(['module:agent-money-in','agent.pin.setup.guard'])->group(function(){
    ...
    //bill pay
    Route::middleware(['module:agent-bill-pay','agent.pin.setup.guard'])->group(function(){
    ...
    //Mobile TopUp
    Route::middleware(['module:agent-mobile-top-up','agent.pin.setup.guard'])->group(function(){
    ...
    //withdraw money
    Route::middleware(['module:agent-withdraw-money','agent.pin.setup.guard'])->group(function(){
    ...
    //add money
    Route::middleware(['module:agent-add-money','agent.pin.setup.guard'])->group(function(){
        Route::controller(AddMoneyController::class)->prefix("add-money")->name("add.money.")->group(function(){
            ...
            //sslcommerz
            Route::post('sslcommerz/success','sllCommerzSuccess')->name('ssl.success')->withoutMiddleware(['auth:agent','verification.guard.agent','agent.google.two.factor','agent.pin.setup.guard']);
            Route::post('sslcommerz/fail','sllCommerzFails')->name('ssl.fail')->withoutMiddleware(['auth:agent','verification.guard.agent','agent.google.two.factor','agent.pin.setup.guard']);
            Route::post('sslcommerz/cancel','sllCommerzCancel')->name('ssl.cancel')->withoutMiddleware(['auth:agent','verification.guard.agent','agent.google.two.factor','agent.pin.setup.guard']);
            //Stripe
            ...
            Route::get('redirect/form/{gateway}', 'redirectUsingHTMLForm')->name('payment.redirect.form')->withoutMiddleware(['auth:agent','verification.guard.agent','agent.google.two.factor','agent.pin.setup.guard']);
            //redirect with Btn Pay
            Route::get('redirect/btn/checkout/{gateway}', 'redirectBtnPay')->name('payment.btn.pay')->withoutMiddleware(['auth:agent','verification.guard.agent','agent.google.two.factor','agent.pin.setup.guard']);
            Route::post("callback/response/{gateway}",'callback')->name('payment.callback')->withoutMiddleware(['web','auth:agent','verification.guard.agent','agent.google.two.factor','agent.pin.setup.guard']);
    ...
    Route::middleware(['module:agent-remittance-money','agent.pin.setup.guard'])->group(function(){
        //Sender Recipient
