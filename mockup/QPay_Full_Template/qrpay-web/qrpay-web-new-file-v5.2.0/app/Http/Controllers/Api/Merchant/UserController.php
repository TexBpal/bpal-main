...
class UserController extends Controller
{
    ...
    public function home(){
        ...
        $data =[
            ...
            'pin_verification'          => $this->basic_settings->merchant_pin_verification,
            ...
            'merchant'                  => $user,
            'totalMoneyOut'             => getAmount($money_out_amount,get_wallet_precision()).' '.get_default_currency_code(),
            'receiveMoney'              => getAmount($receive_money,get_wallet_precision()).' '.get_default_currency_code(),
            'gateway_amount'            => getAmount($gateway_amount,get_wallet_precision()).' '.get_default_currency_code(),
        ...
        $transactions = Transaction::auth()->latest()->get()->map(function($item){
            $statusInfo = [
                ...
                return[
                    ...
                    'transaction_type' => $item->type,
                    'request_amount' => getAmount($item->request_amount,get_wallet_precision()).' '.get_default_currency_code() ,
                    'payable' => getAmount($item->payable,get_wallet_precision()).' '.$item->creator_wallet->currency->code,
                    'exchange_rate' => '1 ' .get_default_currency_code().' = '.getAmount($item->currency->rate,get_wallet_precision()).' '.@$item->currency->currency_code,
                    'total_charge' => getAmount($item->charge->total_charge,get_wallet_precision()).' '.$item->creator_wallet->currency->code,
                ...
                return[
                    ...
                    'card_amount' => getAmount(@$item->details->card_info->amount,get_wallet_precision()).' '.get_default_currency_code(),
    ...
    public function getRemainingBalance(){
        ...
        try{
            $result = (new TransactionLimit())->trxLimit('merchant_id',$sender_wallet->merchant->id,$validated['transaction_type'],$sender_wallet->currency,$validated['sender_amount'],$limits,$validated['attribute'],'json',$extra_rate);
    ...
    public function pinVerify(Request $request){
        $validator      = Validator::make($request->all(),[
            'pin'       =>  $this->basic_settings->merchant_pin_verification == true ? 'required|digits:4' : 'nullable',
        ]);
        if($validator->fails()){
            $error =  ['error'=>$validator->errors()->all()];
            return Helpers::validation($error);
        ...
        if( $this->basic_settings->merchant_pin_verification == true){
