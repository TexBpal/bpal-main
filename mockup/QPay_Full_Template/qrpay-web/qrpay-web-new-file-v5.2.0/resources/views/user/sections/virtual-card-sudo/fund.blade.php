                                ...
                                <form class="card-form" action="{{ setRoute('user.sudo.virtual.card.fund.confirm') }}" method="POST">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <select class="form--control nice-select currency" name="currency">
                                                ...
                                                <option value="{{ $item->currency_code }}"
                                                    ...
                                                    data-name="{{ $item->country }}">{{ $item->currency_code }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    @if($basic_settings->user_pin_verification == true)
                                            <div class="col-xl-12 col-lg-12">
                                                <a href="javascript:void(0)" class="btn--base w-100 btn-loading fundBtn" data-bs-toggle="modal" data-bs-target="#checkPin">{{ __("Fund Card") }} <i class="las la-plus-circle ms-1"></i></a>
                                            </div>
                                        </div>
...
<script>
    ...
    function getExchangeRate(){
        ...
        $('.f-exchange-rate').html("1 " + card_currency + " = " + parseFloat(rate).toFixed(8) + " " + from_currency);
    ...
    function getPreview() {
        ...
        $('.fees').html( parseFloat(total_charge).toFixed(acceptVar().sPrecison) + " " + from_currency);
        ...
        $('.payable-total').html("{{ __('Payable') }}"+" : " + parseFloat(pay_in_total).toFixed(acceptVar().sPrecison) + " " + from_currency);
    ...
    function get_remaining_limits(){
        ...
        var currency_id         = acceptVar().fCurrencySelected.data('currency-id');
        var sender_amount       = $("input[name=fund_amount]").val();
