...
function get_default_currency($default_currency = null)
{
    if($default_currency == null) $default_currency = CurrencyProvider::default();
    if ($default_currency != false) {
...
function card_currency_name($code)
{
    $currency = ExchangeRate::where('currency_code',$code)->first();
    return  $currency->name ?? get_default_currency_name();
}
...
function makeActiveRouteSlugOnly($routeName, $slug = null)
{
    $class = 'active';
    ...
    if ($slug === null && $currentSlug === null) {
        return $class;
    }

    if ($slug !== null && $slug === $currentSlug) {
        return $class;
    }
...
function authWalletBalance(){
    if(auth()->guard('web')->check()){
        ...
        return get_amount($wallet->balance,null,get_wallet_precision());
    }else if(auth()->guard('merchant')->check()){
...
function virtual_card_system($name)
{
 ...
 function userActiveCardData(){
    $user = auth()->user();
    $cardApi = VirtualCardApi::first();
    $api_mode = $cardApi->config->stripe_mode ?? null;
    $card_mode = GlobalConst::SANDBOX;
    $card_mode = ($api_mode ?? GlobalConst::SANDBOX) === GlobalConst::LIVE ? GlobalConst::LIVE : GlobalConst::SANDBOX;
...
function virtualCardAmountOnBaseCurrency($cards){
    ...
    foreach ($cards as $card) {
        ...
        $exchange_rate = ExchangeRate::where('currency_code',$card->currency)->first()->rate??1;
        $result = $requestAmount / $exchange_rate;
        $totalAmount += $result;
    }
    return $totalAmount??0 ;
...
function get_precision_from_admin(){
    $data =[
        'fiat_precision_value'      => 2,
        'crypto_precision_value'    => 8

    ];
    return $data;
}
function support_currencies(array $supported_currency,$supported_country) {
    if(count($supported_currency) > 0){
        $all_currency = ExchangeRate::get();

        $filtered_currency = $all_currency->filter(function ($currency) use ($supported_currency,$supported_country) {
            return in_array($currency->currency_code, $supported_currency) && in_array($currency->name, $supported_country);
        });
...
function get_flutter_wave_api_data($request_data,$moneyOutData,$callback_url,$reference){
    ...
    if($country->currency_code == "USD"){
        ...
        $api_send_data =[
            "amount"                    => $moneyOutData->will_get ?? $moneyOutData->charges->will_get,
