        ...
        <div class="dashboard-list-item-wrapper">
            ...
            <div class="preview-list-wrapper">
                        ...
                        <div class="preview-list-item">
                            <div class="preview-list-left">
                                <div class="preview-list-user-wrapper">
                                    ...
                                    <div class="preview-list-user-content">
                                        <span>{{ __("Refund Balance") }}</span>
                                    </div>
                                </div>
                            </div>
                            <div class="preview-list-right">
                                <span class=""><a href="javascript:void(0)" class="btn--base refund-balance-make-payment" data-id={{ $item->id }}>{{ __("Refund") }}</a></span>
                            </div>
                        </div>
                        @endif
                    @endif
                ...
                @endif
                 ...
                 @if($item->status == payment_gateway_const()::STATUSSUCCESS && $item->details->env_type == payment_gateway_const()::ENV_PRODUCTION)
                    <div class="preview-list-item">
                        <div class="preview-list-left">
                            <div class="preview-list-user-wrapper">
                                <div class="preview-list-user-icon">
                        ...
                        <div class="preview-list-right">
                            <span class=""><a href="javascript:void(0)" class="btn--base refund-balance-payment-gateway" data-id={{ $item->id }}>{{ __("Refund") }}</a></span>
                        </div>
                    </div>
                    @endif
            @endif
...
@push('script')
    ...
    <script>
        $(".refund-balance-make-payment").click(function(){
            var id = $(this).attr('data-id');
            var actionRoute =  "{{ setRoute('merchant.receive.money.refund.balance.make.payment') }}";
            var target      = id;
            var btnText     = "{{ __('Refund Balance') }}";
            var confirmText = "{{ __('Are you sure to') }}";
            var message = `${confirmText} <strong>${btnText}</strong>?`;
            openAlertModal(actionRoute,target,message,btnText,"POST");
        });
        $(".refund-balance-payment-gateway").click(function(){
            var id = $(this).attr('data-id');
            var actionRoute =  "{{ setRoute('merchant.receive.money.refund.balance.payment.gateway') }}";
            var target      = id;
            var btnText     = "{{ __('Refund Balance') }}";
            var confirmText = "{{ __('Are you sure to') }}";
            var message = `${confirmText} <strong>${btnText}</strong>?`;
